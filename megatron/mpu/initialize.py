...
def initialize_model_parallel(tensor_model_parallel_size_=1,
    ...
    for i in range(num_pipeline_model_parallel_groups):
        ...
        for j in range(len(ranks)):
            ranks_ = [ranks[j], ranks[(j + 1) % len(ranks)]]
            args = get_args()
            if args.use_gloo_for_pipeline_parallel_send_recv:
                group = torch.distributed.new_group(ranks_, backend='gloo')
            else:
...
def get_pipeline_model_parallel_current_rank():
    assert _PIPELINE_GLOBAL_RANKS is not None, \
        "Pipeline parallel group is not initialized"
    rank_in_pipeline = get_pipeline_model_parallel_rank()
    world_size = get_pipeline_model_parallel_world_size()
    return _PIPELINE_GLOBAL_RANKS[rank_in_pipeline % world_size]

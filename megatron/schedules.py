...
def forward_backward_pipelining_with_interleaving(forward_step_func, data_iterator, model,
    ...
    for k in range(num_warmup_microbatches):
        ...
        if mpu.is_pipeline_first_stage(ignore_virtual=True):
            next_forward_model_chunk_id = get_model_chunk_id(k+1+get_num_microbatches()-pipeline_parallel_size, forward=True)
            if next_forward_model_chunk_id == 0:
                recv_prev = False
            elif next_forward_model_chunk_id >= num_model_chunks:
                recv_prev = False
            # if k + 1 < pipeline_parallel_size and k + 1 >= get_num_microbatches() * (num_model_chunks - 1) + pipeline_parallel_size:
            #     recv_prev = False
    ...
    for k in range(num_microbatches_remaining, num_microbatches):
        ...
        if mpu.is_pipeline_last_stage(ignore_virtual=True):
            next_backward_model_chunk_id = get_model_chunk_id(k+1+get_num_microbatches()-pipeline_parallel_size, forward=False)
            if next_backward_model_chunk_id == (num_model_chunks - 1):
                recv_next = False
            elif next_backward_model_chunk_id < 0:
                recv_next = False
        if k == (num_microbatches - 1):
            recv_next = False

    ...
    if tensor_send_prev is not None:
        if args.use_gloo_for_pipeline_parallel_broadcast:
            return torch.distributed.broadcast(tensor_send_prev,
                ...
                async_op=True)
        print("{} send to {}".format(mpu.get_pipeline_model_parallel_current_rank(), mpu.get_pipeline_model_parallel_prev_rank()))
    ...
    if tensor_send_next is not None:
        if args.use_gloo_for_pipeline_parallel_broadcast:
            return torch.distributed.broadcast(tensor_send_next,
                ...
                async_op=True)
        print("{} send to {}".format(mpu.get_pipeline_model_parallel_current_rank(), mpu.get_pipeline_model_parallel_next_rank()))
    ...
    if tensor_recv_prev is not None:
        if args.use_gloo_for_pipeline_parallel_broadcast:
            return tensor_recv_prev, torch.distributed.broadcast(tensor_recv_prev,
                ...
                group=mpu.get_pipeline_model_parallel_prev_rank_group(),
                async_op=True)
        print("{} recv from {}".format(mpu.get_pipeline_model_parallel_current_rank(), mpu.get_pipeline_model_parallel_prev_rank()))
    ...
    if tensor_recv_next is not None:
        if args.use_gloo_for_pipeline_parallel_broadcast:
            return tensor_recv_next, torch.distributed.broadcast(tensor_recv_next,
                ...
                group=mpu.get_pipeline_model_parallel_next_rank_group(),
                async_op=True)
        print("{} recv from {}".format(mpu.get_pipeline_model_parallel_current_rank(), mpu.get_pipeline_model_parallel_next_rank()))
